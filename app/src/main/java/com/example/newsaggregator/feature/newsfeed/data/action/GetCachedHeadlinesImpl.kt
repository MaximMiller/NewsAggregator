...
) : GetCachedHeadlinesAction, GetCachedHeadlinesByCategoryAction {
    override suspend operator fun invoke(): List<NewsItem> =
        dao.getAllNews().map(mapper::entityToDomain)

    override suspend operator fun invoke(category: String): List<NewsItem> =
        dao.getNewsByCategory(category).map(mapper::entityToDomain)
}